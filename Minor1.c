...
{
char names[6][100] = { {"Delhi"},{"Dehradun"},{"Ramnagar"},{"Mussoorie"},{"Nanital"},{"Muzaffarnagar"} };
int i, ncity;
completed[city] = 1;
// if (ctrtsp == 0)
// {
...
int cc = atoi(cost_c);
printf("\t\tThe minimum distance for the above route is %d km.(from the file)",cc);
printf("\n\t\t");
...
int cc = atoi(cost_c);
// printf("%d",cc);
printf("\n\t\tThe minimum distance between the %s and %s is %d km.(from the file)\n\t\t",src,dest,cc);
read_route(src, dest);
printf("\n\t\t");
Calculate_Cost(cc,choice);
break;
...
{
value=distance[i];
printf("\n\t\tDistance between the entered Source And Destination is %d km", distance[i]);
if (i == 0)
{
printf("\n\t\tPath=Delhi");
...
printf("\n\t\tThe shortest distance between %s and %s is %d km\n\t\t", str, str2, res);
...
printf("\n\t\tThe minimum distance for the route is : %d km\n\t\t", cost);
